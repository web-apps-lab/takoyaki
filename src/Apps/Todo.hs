...
newtype TodoList = TodoList [Task] deriving (Show, Generic)
...
todoApp =
  App
    { appDesc = AppDesc "Todo" "A Todo Web App" "",
      ...
      appService = const . const . const . const $ pure ()
    }
  where
...
todoHandleEvent :: WSEvent -> TVar TodoList -> TBQueue () -> DB.Connection -> Logger -> IO [Html ()]
todoHandleEvent wEv appStateV _serviceQ _dbConn _logger = do
...
renderApp :: TVar TodoList -> Logger -> DB.Connection -> IO (Html ())
...
delTask (TodoList tasks) taskId = do
  TodoList $ filter (\task -> (task.taskId) /= taskId) tasks
